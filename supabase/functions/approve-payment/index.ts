// Approve Payment Edge Function - MAINNET
// This function handles payment approval in the Pi Network payment flow
...
serve(async (req) => {
  ...
  try {
    ...
    // Parse request body
    const { paymentId, paymentData } = await req.json();
    ...
    const piResponse = await fetch(`${PI_API_BASE_URL}/v2/payments/${paymentId}/approve`, {
      method: 'POST',
      headers: {
    ...
    if (logError) {
      console.error('Failed to log payment approval:', logError);
    }
