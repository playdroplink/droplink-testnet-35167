...
export const PiProvider = ({ children }: { children: ReactNode }) => {
  ...
  const getDROPBalance = async (): Promise<DropTokenBalance> => {
  ...
  const createDROPTrustline = async (): Promise<boolean> => {
    ...
    try {
      ...
      const paymentResult = await window.Pi.createPayment(trustlinePayment, {
  ...
  const createAccount = async (username: string, displayName?: string): Promise<PiAccount | null> => {
    if (!piUser?.uid) {
      console.error('No Pi user ID available for account creation');
      return null;
    }
    ...
    try {
      ...
      if (data?.success) {
        // Create account object from the response data
      ...
      if (error.message?.includes('USERNAME_EXISTS')) {
        ...
        throw new Error('Failed to create profile. Please check your connection and try again.');
      }
    }
  };
